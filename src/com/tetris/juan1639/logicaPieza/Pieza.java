...
public class Pieza implements IResetControlesEstados
{
	...
	public Pieza(Integer x, Integer y, Integer[][] idPieza, Integer col, Integer filas, Integer[] colorPieza,
			Settings settings)
	{
		this.x = x;
		this.y = y;
	...
	public void dibuja(Graphics g, Integer ancho, Integer alto)
	{
		...
		for (Integer i = rotacion_idPieza; i < fin; i++)
		{
			...
			int[] polX =
			{ x, x + settings.TILE_X, x + settings.TILE_X, x };
			int[] polY =
			{ y, y, y + settings.TILE_Y, y };
			g.fillPolygon(polX, polY, 3);

			g.setColor(new Color(rgb[3], rgb[4], rgb[5]));
			// g.fillRect(x, y, ancho, alto);
			int[] polX2 =
			{ x, x, x + settings.TILE_X, x };
			int[] polY2 =
			{ y, y + settings.TILE_Y, y + settings.TILE_Y, y };
		...
		} else if (sett.controles.isAbajo())
		{
			...
			if (check_colision(sett))
			{
				...
				if (this.y <= sett.Y_INICIAL)
				{
					...
					setAreaTxt(new AreaTexto(64, sett.WIDTH_SCREEN / 4, sett.HEIGHT_SCREEN / 2, "Game Over", rgb));
